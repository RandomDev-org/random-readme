# RandomDevelopment

Proyecto universitario para la asignatura Fundamentos de Ingeniería de Software.

Plataforma colaborativa que conecta a músicos, productores, público y dueños de locales a través de un mapa interactivo de espacios musicales.

## Integrantes

| Nombre | GitHub | Rol |
|:-------|:-------|:----|
| Ignacio Rivera | [@chvrrywavz](https://github.com/chvrrywavz) | Scrum Master |
| Eduardo Blanchard | [@LPadest](https://github.com/LPadest) | Product Owner / Designer |
| Benjamín Isasmendi | [@Mapacheee](https://github.com/Mapacheee) | Developer |
| Jason Monroy | [@Ginger110](https://github.com/Ginger110) | Developer |
| Vicente Tapia | [@xSeVeN-bot](https://github.com/xSeVeN-bot) | Developer |

## Responsabilidades del Equipo

| Integrante | Rol | Responsabilidades                                                                                        |
|:-----------|:----|:---------------------------------------------------------------------------------------------------------|
| Ignacio Rivera | Scrum Master | Historias de usuario como GitHub Issues, Trazabilidad entre artefactos, Responsabilidades identificables |
| Eduardo Blanchard | Product Owner / Designer | Especificación clara de requisitos, Mockups/Prototipos de baja fidelidad, Decisiones de diseño           |
| Benjamín Isasmendi | Developer | Arquitectura, Modelo de Dominio, Justificación de decisiones técnicas y trade-offs                       |
| Jason Monroy | Developer | ReqExtrafuncionales, Análisis de trade-offs, Impacto de cambios en requerimientos                        |
| Vicente Tapia | Developer | Solución y Problemática articuladas, Consistencia entre artefactos                                       

## Historias de Usuario

| ID    | Nombre                                                           | Issue                                                           |
|:------|:-----------------------------------------------------------------|:----------------------------------------------------------------|
| US-01 | Registro y gestión de espacios culturales municipales            | [#6](https://github.com/RandomDev-org/random-readme/issues/6)   |
| US-02 | Panel de estadísticas y métricas de interacción                  | [#7](https://github.com/RandomDev-org/random-readme/issues/7)   |
| US-03 | Seguimiento de talentos y monitoreo de redes de artistas         | [#8](https://github.com/RandomDev-org/random-readme/issues/8)   |
| US-04 | Sistema de geovigilancia de tendencias y chat con músicos        | [#9](https://github.com/RandomDev-org/random-readme/issues/9)   |
| US-05 | Verificación y administración técnica de locales                 | [#10](https://github.com/RandomDev-org/random-readme/issues/10) |
| US-06 | Gestión de eventos y cartelera de jam sessions                   | [#11](https://github.com/RandomDev-org/random-readme/issues/11) |
| US-07 | Búsqueda y filtrado de locales por ubicación y género            | [#12](https://github.com/RandomDev-org/random-readme/issues/12) |
| US-08 | Consulta de programación musical detallada                       | [#13](https://github.com/RandomDev-org/random-readme/issues/13) |
| US-09 | Localización de puntos de encuentro y confirmación de asistencia | [#14](https://github.com/RandomDev-org/random-readme/issues/14) |
| US-10 | Contribución comunitaria y gestión colaborativa de eventos       | [#15](https://github.com/RandomDev-org/random-readme/issues/15) |

## Documentación

- [Historias de usuario](https://github.com/RandomDev-org/random-readme/issues)
- [Problemática](docs/problem.md)
- [Solución](docs/solution.md)
- [Arquitectura del software](docs/architecture.md)
- [Requisitos extrafuncionales](docs/functional-requirements.md)
- [Modelo de dominio](docs/domain-model.md)
- [Prototipos Figma](https://try-step-50438657.figma.site/)
